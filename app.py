...
try:
    ...
    def tela_usuarios():
        st.title("👤 Usuários")
        st.info("Tela de usuários em desenvolvimento.")
...
if len(menu_opcoes) <= 1:
    st.error("⛔ Usuário sem permissões.")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown("<div class='sidebar-title'>ERP Verde Infância</div>", unsafe_allow_html=True)
...
def bloquear(permissao):
    if not (admin_total or st.session_state.get(permissao, False)):
        st.error("⛔ Você não possui permissão.")
        st.stop()
